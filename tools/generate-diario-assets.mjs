...
async function generateImages(item, article, paths, imgSkinFull) {
  ...
  const jobs = [
    { key: "hero", out: paths.hero, prompt: prompts.hero || item.hero_brief, w: imgSkin.hero.width, h: imgSkin.hero.height },
    { key: "fig1", out: paths.figures[0], prompt: prompts.fig1 || article.figures?.[0]?.alt, w: imgSkin.figure.width, h: null },
    { key: "fig2", out: paths.figures[1], prompt: prompts.fig2 || article.figures?.[1]?.alt, w: imgSkin.figure.width, h: null },
  ];

  for (const job of jobs) {
    const fullPrompt = `${prefix}. ${job.prompt}. ${imgSkin.disclosure}. ${imgSkin.negative}`;
    ...
    pngToWebp(pngTmp, webpOut, job.w, job.h, job.key === "hero" ? 60 : 65);
  }
}
